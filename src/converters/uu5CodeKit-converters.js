    ...
    this._converters = [
      ...
      // TODO CodeKit by bylo idealni vytahnout do pluginu
      {
        filter: function (node) {
          let res = this.checkTag('UU5.Codekit.CodeViewer', ['mode', 'key'])(node);

          if (res) {
            res = this.checkTag('uu5string.pre')(Array.prototype.filter.call(node.childNodes, (child) => child.nodeType === 1).find(() => true));
          }
          return res;
        },
        replacement: function (content, node) {
          var mode = node.getAttribute('mode');
          let uu5preContent = Array.prototype.filter.call(node.childNodes, (child) => child.nodeType === 1).find(() => true).textContent;
